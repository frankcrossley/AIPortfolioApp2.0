import React, { useState } from 'react';
import {
  TrendingUp,
  ArrowRight,
  ExternalLink,
} from 'lucide-react';
import { usePortfolio } from '../context/PortfolioContext';
import { Header } from '../components/Header';

export const OverviewView: React.FC = () => {
  const {
    metrics,
    gaps,
    compositionByType,
    investmentByDepartment,
    compositionByLifecycle,
    applyGapFilter,
    setActiveNav,
    setFilters,
  } = usePortfolio();

  const [compositionTab, setCompositionTab] = useState<'type' | 'department' | 'lifecycle'>('type');

  // Format currency helpers
  const formatCostInMillions = (val: number) => {
    return `£${(val / 1000000).toFixed(1)}m`;
  };

  // Helper for SVG donut
  const renderDonutArcs = () => {
    const total = compositionByType.reduce((sum, item) => sum + item.count, 0) || 1;
    let accumulatedAngle = 0;
    const radius = 68;
    const strokeWidth = 26;
    const center = 85;

    return compositionByType.map((item, idx) => {
      const angle = (item.count / total) * 360;
      const startAngle = accumulatedAngle;
      const endAngle = accumulatedAngle + angle;
      accumulatedAngle = endAngle;

      if (item.count === 0) return null;

      // Convert angles to radians
      const startRad = ((startAngle - 90) * Math.PI) / 180;
      const endRad = ((endAngle - 90) * Math.PI) / 180;

      const x1 = center + radius * Math.cos(startRad);
      const y1 = center + radius * Math.sin(startRad);
      const x2 = center + radius * Math.cos(endRad);
      const y2 = center + radius * Math.sin(endRad);

      const largeArcFlag = angle > 180 ? 1 : 0;

      const pathData = [
        `M ${x1} ${y1}`,
        `A ${radius} ${radius} 0 ${largeArcFlag} 1 ${x2} ${y2}`,
      ].join(' ');

      return (
        <path
          key={idx}
          d={pathData}
          fill="none"
          stroke={item.color}
          strokeWidth={strokeWidth}
          className="transition-all duration-300 hover:opacity-80 cursor-pointer"
          onClick={() => {
            const mappedType = item.type === 'Experiments' ? 'Experiment' : item.type.replace(/s$/, '');
            setFilters((prev) => ({ ...prev, type: mappedType }));
            setActiveNav('portfolio');
          }}
        >
          <title>{`${item.type}: ${item.count} (${item.percentage}%)`}</title>
        </path>
      );
    });
  };

  return (
    <div className="flex-1 flex flex-col min-w-0 bg-[#f8fafc] overflow-y-auto">
      <Header
        title="Portfolio Overview"
        subtitle="A clear view of your AI estate, investment and value evidence."
      />

      <div className="p-8 max-w-7xl w-full mx-auto space-y-6">
        {/* Top 4 KPI Cards matching screenshot */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* KPI 1 */}
          <div
            id="kpi-total-initiatives"
            onClick={() => setActiveNav('portfolio')}
            className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs hover:border-blue-300 hover:shadow-sm transition-all cursor-pointer group"
          >
            <div className="text-xs font-medium text-slate-500 flex items-center justify-between">
              <span>Total AI Initiatives</span>
              <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-600 transition-colors" />
            </div>
            <div className="text-3xl font-bold text-slate-900 mt-2 tracking-tight">
              {metrics.totalInitiatives}
            </div>
            <div className="text-xs text-emerald-600 font-medium mt-2 flex items-center gap-1">
              <TrendingUp className="w-3 h-3" />
              <span>↑ 3 since last month</span>
            </div>
          </div>

          {/* KPI 2 */}
          <div
            id="kpi-annual-cost"
            className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs"
          >
            <div className="text-xs font-medium text-slate-500">
              Annual estimated cost
            </div>
            <div className="text-3xl font-bold text-slate-900 mt-2 tracking-tight">
              {formatCostInMillions(metrics.annualEstimatedCost)}
            </div>
            <div className="text-xs text-slate-500 mt-2 font-medium">
              {formatCostInMillions(metrics.confirmedCost)} confirmed |{' '}
              {formatCostInMillions(metrics.estimatedCost)} estimated
            </div>
          </div>

          {/* KPI 3 */}
          <div
            id="kpi-production-agents"
            onClick={() => {
              setFilters((prev) => ({ ...prev, type: 'Agent', lifecycleStage: 'Production' }));
              setActiveNav('portfolio');
            }}
            className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs hover:border-blue-300 hover:shadow-sm transition-all cursor-pointer group"
          >
            <div className="text-xs font-medium text-slate-500 flex items-center justify-between">
              <span>Production agents</span>
              <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-600 transition-colors" />
            </div>
            <div className="text-3xl font-bold text-slate-900 mt-2 tracking-tight">
              {metrics.productionAgents}
            </div>
            <div className="text-xs text-emerald-600 font-medium mt-2 flex items-center gap-1">
              <TrendingUp className="w-3 h-3" />
              <span>↑ 2 since last month</span>
            </div>
          </div>

          {/* KPI 4 */}
          <div
            id="kpi-value-evidence"
            className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs"
          >
            <div className="text-xs font-medium text-slate-500">
              Value evidence available
            </div>
            <div className="text-3xl font-bold text-slate-900 mt-2 tracking-tight">
              {metrics.valueEvidenceCount}
            </div>
            <div className="text-xs text-slate-500 mt-2 font-medium">
              {Math.round((metrics.valueEvidenceCount / (metrics.totalInitiatives || 1)) * 100)}% of initiatives
            </div>
          </div>
        </div>

        {/* 2 Main Dashboard Sections matching screenshot */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Left: Portfolio Composition */}
          <div className="lg:col-span-7 bg-white rounded-xl border border-slate-200/80 p-6 shadow-xs flex flex-col">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-slate-100">
              <div>
                <h2 className="text-sm font-bold text-slate-900">Portfolio composition</h2>
                <p className="text-xs text-slate-500">Distribution across current active inventory</p>
              </div>

              {/* Pill Tabs */}
              <div className="inline-flex p-0.5 bg-slate-100 rounded-lg border border-slate-200 text-xs font-medium">
                <button
                  onClick={() => setCompositionTab('type')}
                  className={`px-3 py-1 rounded-md transition-all ${
                    compositionTab === 'type'
                      ? 'bg-white text-slate-900 shadow-xs font-semibold'
                      : 'text-slate-600 hover:text-slate-900'
                  }`}
                >
                  By AI type
                </button>
                <button
                  onClick={() => setCompositionTab('department')}
                  className={`px-3 py-1 rounded-md transition-all ${
                    compositionTab === 'department'
                      ? 'bg-white text-slate-900 shadow-xs font-semibold'
                      : 'text-slate-600 hover:text-slate-900'
                  }`}
                >
                  By department
                </button>
                <button
                  onClick={() => setCompositionTab('lifecycle')}
                  className={`px-3 py-1 rounded-md transition-all ${
                    compositionTab === 'lifecycle'
                      ? 'bg-white text-slate-900 shadow-xs font-semibold'
                      : 'text-slate-600 hover:text-slate-900'
                  }`}
                >
                  By lifecycle
                </button>
              </div>
            </div>

            {/* Tab 1: By AI Type (Interactive Donut) */}
            {compositionTab === 'type' && (
              <div className="pt-6 flex flex-col md:flex-row items-center justify-around gap-6 flex-1">
                {/* SVG Donut */}
                <div className="relative w-44 h-44 shrink-0 flex items-center justify-center">
                  <svg viewBox="0 0 170 170" className="w-full h-full transform -rotate-90">
                    {renderDonutArcs()}
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
                    <span className="text-2xl font-bold text-slate-900 leading-none">
                      {metrics.totalInitiatives}
                    </span>
                    <span className="text-[11px] text-slate-400 font-medium mt-0.5">
                      initiatives
                    </span>
                  </div>
                </div>

                {/* Legend matching screenshot */}
                <div className="flex-1 w-full space-y-2.5 max-w-xs">
                  {compositionByType.map((item) => (
                    <button
                      key={item.type}
                      onClick={() => {
                        const mappedType = item.type === 'Experiments' ? 'Experiment' : item.type.replace(/s$/, '');
                        setFilters((prev) => ({ ...prev, type: mappedType }));
                        setActiveNav('portfolio');
                      }}
                      className="w-full flex items-center justify-between text-xs py-1 px-2 rounded-md hover:bg-slate-50 transition-colors group"
                    >
                      <div className="flex items-center gap-2">
                        <span
                          className="w-2.5 h-2.5 rounded-full shrink-0"
                          style={{ backgroundColor: item.color }}
                        />
                        <span className="text-slate-700 font-medium group-hover:text-blue-600 transition-colors">
                          {item.type}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-slate-900">{item.count}</span>
                        <span className="text-slate-400 text-[11px] w-10 text-right">
                          ({item.percentage}%)
                        </span>
                      </div>
                    </button>
                  ))}
                </div>
              </div>
            )}

            {/* Tab 2: By Department */}
            {compositionTab === 'department' && (
              <div className="pt-6 space-y-3.5 flex-1">
                {investmentByDepartment.map((dept) => {
                  const maxCost = investmentByDepartment[0]?.cost || 1;
                  const pct = Math.round((dept.cost / maxCost) * 100);
                  return (
                    <div
                      key={dept.department}
                      onClick={() => {
                        setFilters((prev) => ({ ...prev, department: dept.department }));
                        setActiveNav('portfolio');
                      }}
                      className="group cursor-pointer"
                    >
                      <div className="flex justify-between text-xs mb-1">
                        <span className="font-medium text-slate-700 group-hover:text-blue-600">
                          {dept.department}
                        </span>
                        <div className="flex gap-2">
                          <span className="text-slate-400 font-normal">
                            {dept.count} items
                          </span>
                          <span className="font-semibold text-slate-900">
                            £{(dept.cost / 1000).toLocaleString()}k
                          </span>
                        </div>
                      </div>
                      <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-blue-600 rounded-full transition-all duration-500 group-hover:bg-blue-700"
                          style={{ width: `${Math.max(pct, 6)}%` }}
                        />
                      </div>
                    </div>
                  );
                })}
              </div>
            )}

            {/* Tab 3: By Lifecycle */}
            {compositionTab === 'lifecycle' && (
              <div className="pt-6 space-y-3.5 flex-1">
                {compositionByLifecycle.map((stage) => {
                  let barColor = 'bg-slate-400';
                  if (stage.stage === 'Production') barColor = 'bg-emerald-500';
                  else if (stage.stage === 'Development' || stage.stage === 'In progress') barColor = 'bg-blue-500';
                  else if (stage.stage === 'Pilot') barColor = 'bg-amber-500';
                  else if (stage.stage === 'Evaluation') barColor = 'bg-purple-500';

                  return (
                    <div
                      key={stage.stage}
                      onClick={() => {
                        setFilters((prev) => ({ ...prev, lifecycleStage: stage.stage }));
                        setActiveNav('portfolio');
                      }}
                      className="group cursor-pointer"
                    >
                      <div className="flex justify-between text-xs mb-1">
                        <span className="font-medium text-slate-700 group-hover:text-blue-600">
                          {stage.stage}
                        </span>
                        <div className="flex gap-2">
                          <span className="font-semibold text-slate-900">
                            {stage.count} items
                          </span>
                          <span className="text-slate-400">({stage.percentage}%)</span>
                        </div>
                      </div>
                      <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full ${barColor} rounded-full transition-all duration-500`}
                          style={{ width: `${Math.max(stage.percentage, 5)}%` }}
                        />
                      </div>
                    </div>
                  );
                })}
              </div>
            )}
          </div>

          {/* Right: Information Gaps */}
          <div className="lg:col-span-5 bg-white rounded-xl border border-slate-200/80 p-6 shadow-xs flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between pb-5 border-b border-slate-100">
                <div>
                  <h2 className="text-sm font-bold text-slate-900">Information gaps</h2>
                  <p className="text-xs text-slate-500">Uncovered metadata and compliance gaps</p>
                </div>
                <button
                  id="view-all-gaps-btn"
                  onClick={() => {
                    applyGapFilter('all-gaps');
                  }}
                  className="text-xs font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1 hover:underline"
                >
                  View all
                  <ExternalLink className="w-3 h-3" />
                </button>
              </div>

              {/* Gap List Items matching screenshot */}
              <div className="divide-y divide-slate-100 mt-2">
                <button
                  id="gap-no-owner"
                  onClick={() => applyGapFilter('no-owner')}
                  className="w-full py-3 px-2 flex items-center justify-between text-xs hover:bg-slate-50 rounded-lg transition-colors group text-left"
                >
                  <span className="text-slate-700 font-medium group-hover:text-blue-600">
                    No owner
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100">
                      {gaps.missingOwner.length}
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-600 transition-transform group-hover:translate-x-0.5" />
                  </div>
                </button>

                <button
                  id="gap-estimated-cost"
                  onClick={() => applyGapFilter('estimated-cost-only')}
                  className="w-full py-3 px-2 flex items-center justify-between text-xs hover:bg-slate-50 rounded-lg transition-colors group text-left"
                >
                  <span className="text-slate-700 font-medium group-hover:text-blue-600">
                    Estimated cost only
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100">
                      {gaps.estimatedCostOnly.length}
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-600 transition-transform group-hover:translate-x-0.5" />
                  </div>
                </button>

                <button
                  id="gap-missing-outcome"
                  onClick={() => applyGapFilter('no-outcome')}
                  className="w-full py-3 px-2 flex items-center justify-between text-xs hover:bg-slate-50 rounded-lg transition-colors group text-left"
                >
                  <span className="text-slate-700 font-medium group-hover:text-blue-600">
                    No intended outcome
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100">
                      {gaps.missingOutcome.length}
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-600 transition-transform group-hover:translate-x-0.5" />
                  </div>
                </button>

                <button
                  id="gap-no-platform"
                  onClick={() => applyGapFilter('no-platform')}
                  className="w-full py-3 px-2 flex items-center justify-between text-xs hover:bg-slate-50 rounded-lg transition-colors group text-left"
                >
                  <span className="text-slate-700 font-medium group-hover:text-blue-600">
                    No platform relationship
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100">
                      {gaps.noPlatform.length}
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-600 transition-transform group-hover:translate-x-0.5" />
                  </div>
                </button>

                <button
                  id="gap-not-updated-90-days"
                  onClick={() => applyGapFilter('not-updated-90-days')}
                  className="w-full py-3 px-2 flex items-center justify-between text-xs hover:bg-slate-50 rounded-lg transition-colors group text-left"
                >
                  <span className="text-slate-700 font-medium group-hover:text-blue-600">
                    Not updated in 90 days
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100">
                      {gaps.notUpdated90Days.length}
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-600 transition-transform group-hover:translate-x-0.5" />
                  </div>
                </button>
              </div>
            </div>

            {/* Defensibility Notice */}
            <div className="mt-6 p-3 bg-slate-50 rounded-lg border border-slate-200/80 text-[11px] text-slate-600 flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0 mt-1.5" />
              <span>
                Resolving data gaps directly increases AI governance audit scores and defensibility for executive committees.
              </span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
